...
$(function(){
      ...
      /* total menu apply */
        ...
        const ill_btn = $('.g_gnb > ul > li:nth-child(2) a');
        const photo_btn = $('.g_gnb > ul > li:nth-child(3) a');
        const UXUI_btn = $('.g_gnb > ul > li:last-child > a');

        total_mnu.click(function(){
            $('.total').hide();
            $('.total').fadeIn();
            $('#design > ul > li:last-child').css('transform','translate(0px,0px)');
            $('#design > h3').css('opacity','1');
        });

        ill_btn.click(function(){
            $('.total').hide();
            // $('.illust').show();
            $('.illust').fadeIn();
            $('#design > ul > li:last-child').css('transform','translate(-10px,-155px)');
            $('#design > h3').css('opacity','0');
            return false;
        });
      ...
      //목록안의 img태그의 src값 불러오기
        ...
        //좌,우 버튼 클릭시 각각 해당하는 인덱스 값을 구하여 이미지 변경하게 하기
        
        let i_num = $(this).index()+1; //숫자가 1부터 나오도록 한다.
        console.log(i_num);

        $('i.fa-angle-left').click(function(){ //1,11,10,9...
            if(i_num==1){
                i_num=11;
            }else{
            i_num--;
        ...
        $('i.fa-angle-right').click(function(){ //1,2,3,4...
            if(i_num==11){
                i_num=1;
            }else{
                i_num++;
        ...
        function img_check(){
                $('.modal img').attr('src','./images/img'+i_num+'.jpg'); //jpg 확장자를 붙여준다.
        }
